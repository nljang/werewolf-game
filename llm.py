...
class MockLLMProvider(LLMProvider):
    ...
    def json(self, messages: List[Dict[str, str]]) -> Dict[str, Any]:
        ...
        if "scores" in message_content and "suspicion" in message_content and "alive_players" in message_content:
            # This is a leaderboard request - generate dynamic scores
            import json
            import random
            # Extract alive players from the context
            try:
                for msg in messages:
                    content = msg.get("content", "")
                    if "Context JSON:" in content:
                        # Find and parse the JSON context
                        ...
                        if json_start >= 0 and json_end > json_start:
                            ...
                            if alive_players:
                                # Generate more realistic suspicion scores
                                scores = []
                                # Use a seed based on alive players for consistency but variety
                                seed_val = sum(ord(c) for p in alive_players for c in p)
                                ...
                                for i, player in enumerate(alive_players):
                                    ...
                                    # Add some randomness
                                    suspicion = base_suspicion + random.uniform(-0.1, 0.3)
